...
@Rollback(false)
@DataJpaTest
class AlbumRepositoryJpaTest(entityManager: EntityManager, albumJpa: AlbumJpa, groupJpa: GroupJpa) : StringSpec() {
    ...
    init {
        ...
        "find all albums returning raw albums projection" {
            ...
            val result = albumRepositoryJpa.findAll(AlbumRaw::class).unsafeRunSync()
        ...
        "find all albums returning albums with groups projection" {
            val album = factory.insertAlbum()
            val result = albumRepositoryJpa.findAll(AlbumWithGroup::class).unsafeRunSync()
        ...
        "find by group id" {
            ...
            factory.insertAlbum()
            val result = albumRepositoryJpa.findByGroupId(album1.group.id, AlbumWithGroup::class).unsafeRunSync()
